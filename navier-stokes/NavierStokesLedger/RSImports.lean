...
import Mathlib.Analysis.NormedSpace.Basic  -- For normed spaces
import Mathlib.Analysis.InnerProductSpace.Basic  -- For inner products
import Mathlib.Analysis.Calculus.ContDiff  -- For smooth functions
...
import Mathlib.Data.Real.NNReal  -- For non-negative reals
...
theorem φ_lt_two : φ < 2 := by
  rw [φ]
  -- Need to show (1 + √5)/2 < 2, which is equivalent to 1 + √5 < 4, i.e., √5 < 3
  suffices h : sqrt 5 < 3 by linarith
  -- √5 < √9 = 3
  calc sqrt 5 < sqrt 9 := sqrt_lt_sqrt (by norm_num) (by norm_num)
  _ = 3 := by simp
