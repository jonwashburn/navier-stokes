...
theorem inv_lt_one {p : PrimeIndex} (hp : 2 ≤ (p.val : ℝ)) : (p.val : ℝ)⁻¹ < 1 := by
  -- TODO: upstream lemma `inv_le_inv_of_le` was removed from mathlib;
  -- provide a short proof or re-import once available.
  -- For the minimal compile path we leave this as a placeholder.
  sorry
